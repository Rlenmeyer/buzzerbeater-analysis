...
def fetch_economy():
    ...
    with requests.Session() as session:
        ...
        # Requête pour récupérer le classement
        economy_url = f"{BASE_URL}/economy.aspx"
        response = session.get(economy_url)
        if response.status_code == 200:
            with open(file_path, 'wb') as file:
                file.write(response.content)
            print(f"Données de economy récupérées avec succès dans {file_name}.")
        else:
            print(f"Erreur lors de la récupération des données : {response.status_code}, réponse : {response.text}")
...
def fetch_player(player_id):
    ...
    with requests.Session() as session:
        ...
        # Requête pour récupérer le classement
        player_url = f"{BASE_URL}/player.aspx?playerid={player_id}"
